    ...
    NH: NoiseHasher + ?Sized,
{
    ...
    let mut origin = seed_point.into_array();
    let mut distance = distance_function(&point.into_array(), &origin);
    ...
    let value = match return_type {
        ...
        ReturnType::Value => {
            hasher.hash(&seed_cell.into_array()) as f64 / 255.0
        }
    };
    ...
    NH: NoiseHasher + ?Sized,
{
    ...
    let seed_point = get_point(seed_index, near);
    let mut distance =
        distance_function(&point.into_array(), &seed_point.into_array());
